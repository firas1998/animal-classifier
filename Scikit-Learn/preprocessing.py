...
def get_least(classes):
    """
    function that returns the size of the shortest list

    :param classes: a list of lists
    :return: the size of the shortest list in the lists
    """
    i = sys.maxsize
...
def shuffle(animals, labels):
    """
    function that shuffles two arrays in the same manner

    :param animals: array
    :param labels: array
    :return: the 2 arrays shuffled in the same way
    """
...
def read_image(image_path):
    """
    function that reads an image in grayscale, resizes it, flattens the output array, converts it to numpy array, then returns the array

    :param image_path: path to the image to read
    :return: numpy array of the image values
    """

    img = cv2.imread(image_path, 0)  #read image in grayscale with OpenCV
    img_arr = Image.fromarray(img)
    img_resized = img_arr.resize((50, 50))  #resize image
    flat_pic = np.array(img_resized).reshape(50 * 50) #flattens resized image to be a vector, and converts it to numpy array
...
save_images("C:/Users/feras/Desktop/Semester 6/thesis items/data set/animals/raw-img/", "train_images", "train_labels", "test_images", "test_labels")